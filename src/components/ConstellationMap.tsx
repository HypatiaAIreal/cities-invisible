...
export default function ConstellationMap({ activeCategory, selectedCity, onSelectCity }: Props) {
  ...
  // Animate viewBox when category changes — imperative for performance
  useEffect(() => {
    ...
    if (!activeCategory) {
      ...
      const pad = 6;
      let w = Math.max(bboxW + pad * 2, 18);
      let h = Math.max(bboxH + pad * 2, 18);

      // Enforce 100:85 aspect ratio so interpolation stays smooth
      const aspect = 100 / 85;
      if (w / h > aspect) {
        h = w / aspect;
      } else {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      >
        ...
        {/* Constellation lines — always rendered */}
        {constellationLines.map((line, i) => {
          ...
          return (
            <line
              ...
              stroke={line.color}
              strokeWidth="0.1"
              opacity={active ? (activeCategory ? 0.45 : 0.18) : 0.04}
              style={{ transition: "opacity 500ms ease-out" }}
            />
          );
        })}

        {/* All city stars — always rendered, ghost when inactive */}
        {cities.map((city) => {
      ...
      <div className="md:hidden space-y-5 px-2 py-2 overflow-y-auto">
